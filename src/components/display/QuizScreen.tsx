...
export default function QuizScreen() {
  ...
  return (
    <div className="flex w-full max-w-4xl flex-col items-center gap-6 text-center">
      ...
      <div className="w-full rounded-3xl bg-white/90 p-10 shadow-2xl">
        <p className="font-display text-6xl text-gray-800">{question.promptText}</p>
      </div>

      {screen === 'q2' && (
        <div className="w-full rounded-3xl bg-yellow-300 p-8 shadow-2xl">
          <p className="font-head text-xl text-pink-700">정답</p>
          <p className="wordart wordart-blue text-6xl">{question.answerText}</p>
        </div>
      )}
    </div>
  )
