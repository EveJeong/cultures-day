...
export default function AdminPage() {
  ...
  if (!isFirebaseConfigured) {
    return <Shell><p className="text-white">⚠️ Firebase 미설정</p></Shell>
  }
  if (!state || !game) {
    return <Shell><p className="font-head text-white">불러오는 중…</p></Shell>
  }
...
function ScorePanel({ game, teams }: { game: Game; teams: Team[] }) {
  ...
  if (game.scoringType === 'rank') {
    return <RankControl game={game} teamName={teamName} />
  }

  if (game.scoringType === 'free') {
    return <FreeControl game={game} teamName={teamName} />
  }

  return (
    <p className="font-body text-sm text-gray-400">
      이 게임({game.engineType})은 진행 엔진(퀴즈/제시어) 화면에서 점수가 부여됩니다. (구현 예정)
    </p>
  )
}
  ...
  return (
    <div className="space-y-2">
      ...
      <button
        ...
        onClick={() => awardRank(game, ranks, round)}
      >
        순위 확정 ({round})
      </button>
    </div>
      ...
      <button
        ...
        onClick={() =>
          awardFree(
            game,
            Object.fromEntries(
              TEAM_IDS.map((id) => [id, Number(pts[id]) || 0]),
